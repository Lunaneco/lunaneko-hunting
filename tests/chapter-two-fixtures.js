...
export function levelThirtyProfile({level=30,tree=false,cleared=7}={}){
 return normalizeProgression({story:{version:2,actClears:Array.from({length:8},(_,i)=>i<cleared),tsukinekoUnlocked:cleared>=4},characters:Object.fromEntries(['nyanluna','tsukineko','omsolo'].map(id=>[id,{level,breaks:level>=30?1:0,tree:tree?TALENT_NODES.map(n=>n.id):[]}]))},HEROES);
}
...
export function prepareWithEarnedRewards(profile){
 ...
 while(['nyanluna','tsukineko'].some(id=>p.characters[id].level<30)&&runs<18){
  for(const id of ['nyanluna','tsukineko']){
   ...
   for(const node of TALENT_NODES)unlockTalent(p,id,node.id);
  }
