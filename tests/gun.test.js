...
test('a rifle round credits Tsukineko after switching to Nyanluna before impact',()=>{
  const g=prepare(),e=g.spawnEnemy('moss',0,11);e.hp=1;e.speed=0;g.attackFrom(g.player,1);g.switchHero();g.player.attack=999;for(let i=0;i<25;i++)g.tick(1/60);assert.equal(g.progressFor(1).xp,3);assert.equal(g.progressFor(0).xp,0);
});
