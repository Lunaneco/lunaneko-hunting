...
function quiet(act=0,difficulty='normal'){
 const g=new Adventure({act,difficulty,seed:2,progression:levelThirtyProfile({tree:true,cleared:8})});
...
test('all eight strong bosses have exactly twice normal HP and contact damage in every wave and mode',()=>{
 for(const act of ACTS)for(const difficulty of ['normal','hard'])for(const wave of [2,4,6]){
  ...
  near(normal.maxHp,act.bossHp*hard);near(normal.damage,(act.id>=4?45:22)*hard);near(elite.maxHp,normal.maxHp*2);near(elite.damage,normal.damage*2);assert.equal(elite.speed,normal.speed);
 }
});
...
test('each fork spawns a doubled boss only on the strong route, without changing ordinary enemies',()=>{
 for(const act of ACTS)for(const route of ['safe','elite']){
  ...
  assert.equal(last.elite,route==='elite');if(route==='elite'){assert.equal(last.type,'boss');near(last.maxHp,act.bossHp*2);near(last.damage,(act.id>=4?45:22)*2);}
 }
});
