...
const out='audit/limit-tree';await mkdir(out,{recursive:true});
...
async function open(profile,production=false){
 ...
 await page.goto(`http://127.0.0.1:${production?4173:5174}/?v=limit-tree-audit`);await page.waitForSelector('#loading',{state:'detached',timeout:60000});await page.click('#chapter-menu-open');return {page,context};
}
const seed={characters:{nyanluna:{level:20,xp:99999,breaks:0,tree:['origin','guard1']},tsukineko:{level:1,xp:0,breaks:0},future_hero:{level:7,xp:15,breaks:0}},inventory:{limitStone:3,starBud:88,moonDew:10,wardenCore:1}};
try{
 ...
 assert.equal(await page.locator('[data-material-count="limitStone"]').innerText(),'3');assert.match(await page.locator('#limit-next-level').innerText(),/20.*30/);const initial=await saved(page);assert.deepEqual(initial.characters.nyanluna,seed.characters.nyanluna);
 ...
 await page.setViewportSize({width:390,height:844});await page.click('[data-tree-node="limit50"]');assert.equal(await page.locator('[data-unlock-node="limit50"]').isDisabled(),true);assert.match(await page.locator('.talent-requirements').innerText(),/限界突破 II/);assert.deepEqual(await saved(page),initial);
 ...
 let p=await saved(page);assert.equal(p.inventory.limitStone,2);assert.equal(p.characters.nyanluna.breaks,1);assert.equal(p.characters.nyanluna.level,30);assert.deepEqual(p.characters.nyanluna.tree,seed.characters.nyanluna.tree);
 assert.equal(await page.locator('[data-unlock-node="limit30"]').isDisabled(),true);assert.equal(await page.locator('[data-tree-node="limit40"]').evaluate(el=>el.classList.contains('available')),true);assert.equal(await page.locator('[data-material-count="limitStone"]').innerText(),'2');
 check('The first cap spends one stone even with a double press and banked XP that reaches Lv.30');
 for(const [id,cap,left] of [['limit40',40,1],['limit50',50,0]]){await page.click(`[data-tree-node="${id}"]`);assert.equal(await page.locator(`[data-unlock-node="${id}"]`).isEnabled(),true);await page.locator(`[data-unlock-node="${id}"]`).tap();p=await saved(page);assert.equal(p.characters.nyanluna.level,cap);assert.equal(p.inventory.limitStone,left);}
 assert.equal(p.characters.nyanluna.xp,0);assert.equal(p.characters.nyanluna.breaks,3);assert.deepEqual(p.characters.tsukineko,initial.characters.tsukineko);assert.deepEqual(p.characters.future_hero,initial.characters.future_hero);assert.deepEqual(p.inventory,{...initial.inventory,limitStone:0});assert.equal(await page.locator('.limit-node.unlocked').count(),3);
 await page.locator('.limit-branch').scrollIntoViewIfNeeded();await page.screenshot({path:`${out}/complete-mobile.png`});check('All three cap steps reach Lv.50 while preserving ability stars, materials and the other characters');
 await page.click('[data-menu-tab="growth"]');assert.equal(await page.locator('#limit-stones').innerText(),'0');assert.equal(await page.locator('#growth-nyanluna .growth-level b').innerText(),'50');assert.match(await page.locator('#growth-nyanluna .growth-tree-link').innerText(),/5 \/ 11/);
 ...
 const legacy=await open({characters:{nyanluna:{level:30,xp:20,breaks:1},tsukineko:{level:1}},inventory:{limitStone:0}});await legacy.page.click('[data-menu-tab="growth"]');await legacy.page.click('[data-open-tree="nyanluna"][data-tree-target]');assert.equal(await legacy.page.locator('[data-tree-node="limit40"]').getAttribute('aria-pressed'),'true');assert.equal(await legacy.page.locator('[data-tree-node="limit30"]').evaluate(el=>el.classList.contains('unlocked')),true);assert.match(await legacy.page.locator('.talent-requirements').innerText(),/限界突破石が1個不足/);assert.equal(await legacy.page.locator('[data-unlock-node="limit40"]').isDisabled(),true);
