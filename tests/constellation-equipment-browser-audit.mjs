...
async function shot(name,selector){if(selector)await page.locator(selector).scrollIntoViewIfNeeded();await page.evaluate(async()=>{await document.fonts.ready;await Promise.all([...document.images].filter(e=>e.checkVisibility()).map(e=>e.decode().catch(()=>{})));});await page.screenshot({path:`${out}/${name}.png`});}
