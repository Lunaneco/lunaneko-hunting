...
try{
 ...
 await page.evaluate(async({prefix,base})=>{const cache=await caches.open(prefix+'previous');await cache.put(new URL('assets/voices/nyanluna/nyanluna-df12363a.mp3',base),new Response('obsolete voice'));await cache.put(new URL('assets/voices/tsukineko/tsukineko-ultimate-1-1ebc7498845e.mp3',base),new Response('obsolete Tsukineko voice'));await caches.open('unrelated-app');},{prefix,base:BASE});
 ...
 const keys=await page.evaluate(()=>caches.keys());assert.ok(!keys.includes(prefix+'previous'));assert.ok(keys.includes('unrelated-app'));assert.ok(!files.some(url=>url.endsWith('/nyanluna-df12363a.mp3')));assert.ok(!files.some(url=>url.endsWith('/tsukineko-ultimate-1-1ebc7498845e.mp3')));pass('Previous voice cache is removed while unrelated app caches are preserved');
 ...
 pass('All 35 replacement Tsukineko voices decode offline with the updated playback durations');
