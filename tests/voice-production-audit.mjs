...
try{
 ...
 await page.evaluate(()=>navigator.serviceWorker.ready);const files=await page.evaluate(async()=>{const keys=await caches.keys();const cache=await caches.open(keys.find(k=>k.startsWith('lunaria-v1-')));return(await cache.keys()).map(r=>r.url);});for(const item of Object.values(VOICE_MANIFEST))assert.ok(files.includes(new URL(item.file,BASE).href),`Not cached: ${item.file}`);assert.equal(files.filter(url=>url.includes('/assets/voices/')).length,133);pass('All 133 voices are saved in the correctly scoped offline cache');
 ...
 assert.equal(decoded.length,35);for(const row of decoded){assert.equal(row.channels,1);assert.ok(Math.abs(row.seconds-VOICE_MANIFEST[row.id].duration)<.05,`Voice timing mismatch: ${row.id}`);}
 pass('All 35 Tsukineko voices decode offline with the updated playback durations');
