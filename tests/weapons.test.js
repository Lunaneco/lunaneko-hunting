...
test('the published 75/20/5 rates and equal character odds cover a uniform grid exactly',()=>{
 ...
 for(const id of WEAPON_HERO_IDS){assert.equal(totals[`${id}-2`],225);assert.equal(totals[`${id}-3`],60);assert.equal(totals[`${id}-4`],15);}assert.equal(p.weapons.owned.length,30);assert.equal(p.inventory.weaponTicket,0);
});
...
test('gacha descriptions expose exact odds, locked-character storage and duplicate values and exact-name duplicate rules',()=>{
 ...
 p.inventory.weaponTicket=1;const r=roll(p,.8,.99),result=weaponDrawResult(r,p,true);assert.match(result,/加入後に使えるよう保管/);assert.match(result,/★4/);assert.match(result,/結果を保存/);assert.equal(WEAPON_CATALOG.length,30);
});
...
test('each hero has three distinct combat profiles at every gacha rarity and one starter',()=>{
 assert.equal(new Set(WEAPON_CATALOG.map(w=>w.id)).size,30);
 for(const hero of HEROES)for(const rank of [1,2,3,4]){
