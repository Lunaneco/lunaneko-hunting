...
const firstAct=[
  {id:'meadow-hunt',area:0,metric:'kills',goal:24,name:'草原の影をはらう',description:'星詠みの草原で魔物を累計24体倒す',rewards:{starBud:8}},
  {id:'meadow-crystals',area:0,metric:'crystals',goal:12,name:'草原の光集め',description:'星詠みの草原でクリスタルを累計12個回収',rewards:{moonDew:2}},
  {id:'meadow-clear',area:0,metric:'clears',goal:1,name:'最初の月の門',description:'星詠みの草原の月の門を突破',rewards:{starBud:12}},
  ...
  {id:'ruins-hunt',area:1,metric:'kills',goal:40,name:'眠れる街の見回り',description:'月影の遺跡で魔物を累計40体倒す',rewards:{starBud:16}},
  {id:'ruins-crystals',area:1,metric:'crystals',goal:20,name:'遺跡の光集め',description:'月影の遺跡でクリスタルを累計20個回収',rewards:{moonDew:3}},
  {id:'ruins-clear',area:1,metric:'clears',goal:1,name:'月影を越えて',description:'月影の遺跡の月の門を突破',rewards:{starBud:18}},
  ...
  {id:'dawn-crystals',area:2,metric:'crystals',goal:24,name:'聖域の光集め',description:'暁の聖域でクリスタルを累計24個回収',rewards:{moonDew:4}},
  {id:'dawn-clear',area:2,metric:'clears',goal:1,name:'暁の向こうへ',description:'暁の聖域の最後の月の門を突破',rewards:{starBud:24}},
...
const added=ACTS.slice(1).flatMap(act=>[
  ...act.stages.flatMap((stage,area)=>[
    {id:`act${act.id+1}-${area}-hunt`,act:act.id,area,metric:'kills',goal:act.counts[area*2]+act.counts[area*2+1],name:`${stage.name}の影をはらう`,description:`${stage.name}で魔物を累計${act.counts[area*2]+act.counts[area*2+1]}体倒す`,rewards:{starBud:12+act.id*4}},
    {id:`act${act.id+1}-${area}-crystals`,act:act.id,area,metric:'crystals',goal:20+area*4,name:`${stage.name}の光集め`,description:`${stage.name}でクリスタルを累計${20+area*4}個回収`,rewards:{moonDew:3+act.id}},
    {id:`act${act.id+1}-${area}-clear`,act:act.id,area,metric:'clears',goal:1,name:`${stage.name}を越えて`,description:`${stage.name}の月の門を突破`,rewards:area===2?{wardenCore:1}:{starBud:16+act.id*4}},
    // Separate claims preserve the rewards of old missions and let existing saves earn the new materials.
    ...(act.id>=4?[{id:`act${act.id+1}-${area}-rare-material`,act:act.id,area,metric:'clears',goal:1,name:`${stage.name}・覚醒素材`,description:`${stage.name}の月の門を突破し、この幕の全6WAVEをクリア（冒険モードでも可）`,rewards:area===2?{astralCore:1}:{moonPrism:area+2}}]:[]),
  ]),
