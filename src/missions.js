...
const added=ACTS.slice(1).flatMap(act=>[
  ...act.stages.flatMap((stage,area)=>[
    ...
    ...(act.id>=4?[{id:`act${act.id+1}-${area}-rare-material`,act:act.id,area,metric:'clears',goal:1,name:`${stage.name}・覚醒素材`,description:`${stage.name}の月の門を突破し、この幕の全6WAVEをクリア（冒険モードでも可）`,rewards:area===2?{astralCore:1}:{moonPrism:1}}]:[]),
  ]),
