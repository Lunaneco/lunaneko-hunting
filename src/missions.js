...
const added=ACTS.slice(1).flatMap(act=>[
  ...
  {id:`act${act.id+1}-relic`,act:act.id,area:2,metric:'trials',goal:1,name:['','時の試練','雲海の試練','双星の試練','木漏れ日の試練','翡翠の試練','灰鐘の試練','守り手の試練','桜もちの試練','夢砂糖の試練','こだまの試練','ふぇ〜の約束'][act.id],description:`${actLabel(act.id)}の全6WAVEをチャレンジモード・${[0,180,195,210,230,245,260,280,300,320,340,360][act.id]}秒以内・被弾1回以下でクリア`,rewards:act.id>=8?{astralCore:2,moonPrism:4}:{},equipment:['','clock-pendant','cloud-feather','twin-star-knot','woodland-token','jade-guard','bell-fragment','guardian-knot','mochi-sakura-brooch','mochi-dream-hourglass','mochi-echo-bell-relic','mochi-promise-crown'][act.id],trial:{seconds:[0,180,195,210,230,245,260,280,300,320,340,360][act.id],hits:1,chapter:true}},
  {id:act.id===3?'chapter-master':`act${act.id+1}-master`,act:act.id,area:2,metric:'chapterMaster',goal:1,name:act.id===3?'限界を越えるふたり':`${act.title}・限界試練`,description:`${actLabel(act.id)}の全6WAVEをチャレンジモード・${[0,180,195,210,230,245,260,280,300,320,340,360][act.id]}秒以内・ノーダメージでクリア（単独出撃も可）`,rewards:{limitStone:1},trial:{seconds:[0,180,195,210,230,245,260,280,300,320,340,360][act.id],hits:0,chapter:true}},
]);
