...
export class World {
  ...
  render(game,dt){
    ...
    if(game){
      ...
      this.syncMap(this.bullets,game.projectiles,b=>{if(b.owner==='enemy')return createHostileProjectile(b);const g=new THREE.Mesh(b.kind==='mochiCry'?new THREE.TorusGeometry(.72,.065,6,24):b.kind==='gun'?new THREE.CylinderGeometry(b.ultimate?.1:.065,b.ultimate?.1:.065,b.ultimate?1.7:.78,8):new THREE.SphereGeometry(b.owner==='player'?.16:.27,8,6),material(b.kind==='mochiCry'?0xffbad8:b.kind==='gun'?(b.ultimate?0xe1fbff:0x99efff):b.owner==='player'?0xe2c3ff:0xffa8c7,1.4));if(b.kind==='gun')g.rotation.x=Math.PI/2;g.userData.disposable=true;return g;},(m,b)=>{if(b.owner==='enemy'){updateHostileProjectile(m,b,t);m.position.y+=game.layout.height;return;}m.position.set(b.x,game.layout.height+(b.kind==='gun'?1.45:.9),b.z);if(b.kind==='gun')m.rotation.set(Math.PI/2,0,-Math.atan2(b.vx,b.vz));m.scale.setScalar(1+Math.sin(t*18)*.15);});
