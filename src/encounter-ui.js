...
export function encounterPreparation(act,party,profile,roster){
 ...
 return `<section class="encounter-brief" aria-label="第2章の戦闘と育成目安"><header><strong>適正 Lv.${act.recommendedLevel}</strong><span>穂むすびの国 · 連射と時間差攻撃</span></header><p class="encounter-levels ${ready?'ready':'preparing'}">${members.map(h=>`${h.name} Lv.${h.level}`).join(' ／ ')}<b>${ready?'育成の目安を達成':'Lv.30を目安に育成しよう'}</b></p><p>限界突破で上限をLv.30へ。限界突破石は第1章・第4幕をクリアするたび1個。第1章の再挑戦で経験値と成長素材を集められます。現在のレベルでも出撃できます。</p><button class="secondary" data-prepare-levels>成長ツリーで準備する →</button><div class="encounter-roster">${CHAPTER_TWO_ENEMIES.map(id=>`<span><b>${ENEMY_TYPES[id].name}</b><small>${ENEMY_TYPES[id].role}</small></span>`).join('')}</div><small>通常戦から新しい敵が出現。チャレンジモードでは、ここからさらに敵のHP・攻撃力が30%上がります。</small></section>`;
}
