...
import {weaponDrawResult} from './weapons-ui.js';
...
dialog.append(bar);
let playlist=[],index=0,auto=false,wait=0,finishedAt=-Infinity,returnFocus=null,drawn=null,profile=null,pointer=null;
...
function play(at){
  ...
  summon.start(drawn,{motion:options.motion,quality:options.quality,rng:route?()=>route.roll:Math.random,markup:weaponDrawResult(drawn,profile,true)});
  if(!options.video&&summon.current.video)summon.dropVideo(summon.current);
  const plan=summon.current.plan;
  bar.querySelector('.preview-label').textContent=`${at+1} / ${playlist.length}　${item.weapon.name} ★${item.rarity.rank}　${plan.motion?routeLabel(plan.route):'動きを減らす表示'}`;
  syncBar();
}
function syncBar(){
  bar.classList.toggle('hidden',!summon.finished);bar.dataset.auto=String(auto);
...
document.addEventListener('click',e=>{
  ...
  if(t.dataset.play){returnFocus=t;auto=false;playlist=ITEMS.map(item=>({item}));play(ITEMS.findIndex(item=>item.id===t.dataset.play));}
  ...
  else if(t.dataset.equipWeapon){const hero=t.dataset.weaponHero;if(drawn&&equipWeapon(profile,hero,t.dataset.equipWeapon)){voice.cue(hero,'equip');audio.play('upgrade');summon.updateResult(weaponDrawResult(drawn,profile,true));$('#weapon-draw-result [data-close]')?.focus({preventScroll:true});}}
