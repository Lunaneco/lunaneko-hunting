...
import {ACTS,CHAPTERS,actFor,chapterForAct,actLabel,isActUnlocked,isActCleared,clearTicketReward,nextAct} from './acts.js';
...
import {extraStageSelector,extraDifficultyView} from './extra-stage-ui.js';
...
$('#app').innerHTML=`
  <section id="home" class="home" aria-label="タイトル画面">
    ...
    <div class="home-content"><div class="eyebrow"><span></span> DUO ACTION ROGUELITE</div><h1 class="game-title"><span>ルナネコの</span><span>不思議な冒険</span></h1><div class="title-sub"><i></i>LUNANECO ADVENTURE<i></i></div>
      ...
      <button id="start" class="start-button"><span class="start-icon">${icon('sword')}</span><span><strong>冒険をはじめる</strong><small>メニューで編成・育成・ステージを選ぶ</small></span>${icon('arrow')}</button>
    ...
    <footer class="home-footer"><span>NYANLUNA <i>×</i> TSUKINEKO</span><button id="chapter-menu-open">メニュー・育成</button><button data-open="guide">操作ガイド ${icon('arrow')}</button><span class="version">LUNANECO ADVENTURE / 1.55.0</span></footer>
  </section>
  ...
  <section id="hud" class="hud hidden" aria-label="戦闘情報">
    ...
    <div id="rare-encounter" class="rare-encounter hidden"></div>
...
const dialogs={
  ...
  guide:()=>`<div class="modal-heading"><span class="eyebrow">ADVENTURE GUIDE</span><h2>ふたりなら、もっと遠くへ。</h2><p>移動に集中。攻撃は、仲間にまかせよう。</p></div><div class="guide-grid"><article>${icon('compass')}<h3>撃って、かわして</h3><p><kbd>WASD</kbd> / <kbd>↑↓←→</kbd> または画面をドラッグして移動。近くの敵へ自動攻撃。<kbd>Space</kbd> で無敵時間のある回避。</p></article><article>${icon('swap')}<h3>ふたりの力を重ねる</h3><p>最初はにゃんるなだけで出発。第4幕の最後で親友のつきねこと再会・共闘し、クリアすると正式加入します。加入後はタイトルの「編成・祝福」やメニューの「パーティ編成」で1〜2体を選択。2体編成なら右下の「交代」で、表示された仲間の操作に切り替え。移動しながらでも押せます。<kbd>Q</kbd> または左上の顔でも交代。にゃんるなは遠距離魔法、つきねこは貫通する銃撃。第2章の最後ではオムソロを救出し、クリア後に近接の光剣使いとして加入します。登場人物が増えても出撃は最大2人です。控えの仲間も援護します。HPはキャラ別。援護中の仲間は無敵。倒れると生存中の相方へ自動交代し、全員が倒れると終了します。戦闘不能からの復帰は次の冒険です。</p></article><article>${icon('spark')}<h3>経験値とクリスタル</h3><p>敵を倒したキャラに経験値100%、もう一人の出撃キャラに50%が入り、レベルと基礎能力が成長。援護の撃破も倒した本人に100%。端数の0.5も保存します。戦闘不能でも出撃中なら50%を獲得。未出撃のキャラとLv.50には入りません。落ちたクリスタルを集めると、出撃メンバーに応じた候補から3つの祝福を提示。共通・キャラ由来・2体の連携があり、効果は編成内で共有します。祝福は同じ幕の全6WAVEで維持されます。にゃんるなはスキルダメージ+50%・ゲージ獲得+50%。つきねこは基礎HP・攻撃力・防御力と通常射撃に優れます。</p></article><article>${icon('moon')}<h3>キャラ固有の必殺技</h3><p>攻撃した本人のゲージが増加し、100%で <kbd>E</kbd> または右下の必殺ボタン。にゃんるな「月華の聖域」は範囲攻撃・減速・HP回復。つきねこ「星銃・彗星連射」は貫通弾の8連射。オムソロ「翠光・守り手の円舞」は周囲への連続斬撃と回復・短い無敵。専用イラストとボイスの演出後に発動します。演出中は画面をタップ（PCはEnter／Space）すると省略して即発動します。演出中は敵も味方も止まり、救出の制限時間も進みません。ゲージはキャラ別に保持し、援護でも蓄積。必殺技自体では増えません。</p></article></div><div class="guide-note">全3章・各章4幕・各幕6WAVE。地形はステージごとに変化。2フロアでは下階を制圧して青い階段から上階へ。分岐では左の緑の門が通常、右の赤い門が強ボスと追加素材。強ボスは同じ難易度の通常ボスに対して${ELITE_BOSS_LABEL}。地図を見て歩いて選ぼう。2WAVEごとに月の門が開きます。光る輪へ移動して次へ進もう。各幕の最後の門でクリア。ミッション報酬を受け取り、メニューから次の幕へ進みます。橙の照準線・突進の帯、紫や桃色の魔法陣から離れよう。<br>ゲームパッド：左スティック移動 / A回避 / B交代 / Y必殺 / Start一時停止。<br>祝福・クリスタルは新しい出撃でリセット。キャラ別のレベル・経験値・覚醒の輝石はこのブラウザに自動保存されます。成長ツリーは素材を使ってキャラごとに解放。「スキルの星」では新しい祝福候補を解放し、出撃前に3枠へセット。共通・二人の連携は枠を使わず自動で候補に入ります。星の芽は通常敵から50%で入手。月のしずくは月の門から1個ずつ、守護者の欠片はボスから入手。1段目はレア度1素材のみ。2段目には第2章・チャレンジモードで集めるレア度2素材も必要です。防御力はキャラ固有値＋レベル成長＋ツリー効果で被ダメージを軽減します。初期上限Lv.20。各章の第4幕クリアでもらえる覚醒の輝石と育成素材で、上限を10ずつ最大Lv.50まで解放できます。石は段階ごとに1・2・4個、Lv.40以降はレア度2素材も必要です。上限到達後の経験値も蓄積されます。<br>ステージミッションで育成素材を入手。ユニーク装備は全キャラ共通の各1枠。同じ装備は一人だけ使用でき、装備画面で仲間どうしの付け替えが可能です。チャレンジモードの時間・被弾条件を達成して入手します。専用武器は別枠の★1〜★4。各幕の全6WAVEクリアで毎回ガチャ券を1枚確定入手。ボスからも5%で追加の専用武器ガチャ券が落ち、メニュー「武器ガチャ」で1枚につき1回使えます。4人の専用武器から均等に抽選。★2が75%、★3が20%、★4が5%。★2〜★4は各キャラ・各レア度に性能の違う3種類。装備画面で自由に付け替えられます。同じ武器・同じレア度の重複だけを星の芽に変換。ガチャでは装備を変更しません。高難度の章ミッションでは追加の覚醒の輝石も入手できます。</div><button id="tutorial-replay" class="secondary">にゃんるなと操作を練習する</button><p class="tutorial-replay-note">にゃんるな1体で最初の戦闘から開始。育成・所持品は引き継ぎます。</p><button class="primary" data-close>準備はできた ${icon('arrow')}</button>`,
...
function updatePartyLabels(){
  const chapter=chapterForAct(nextAct(progression));$('.chapter-card .chapter-index').textContent=String(chapter.id+1).padStart(2,'0');$('.chapter-card small').textContent=`CHAPTER 0${chapter.id+1}`;$('.chapter-card h2').textContent=chapter.title;$('.chapter-card p').textContent=chapter.id===2?'最後のもちにゃふぇを救う、全4幕の物語。':chapter.id===1?'こむすびの願いをつなぐ、全4幕の救出劇。':'親友を探して月の世界を巡る、全4幕。';
  ...
  document.querySelectorAll('[data-party-role]').forEach(el=>{const index=Number(el.dataset.partyRole);el.textContent=!isHeroUnlocked(progression,HEROES[index].id)?'未加入':index===selectedHero?'先頭':'援護';});
...
function showResult(victory){
  ...
  if(victory&&(storyEnabled||game.actConfig.extra)){playStory('ending',()=>goMenu({score,earnedXp,earnedMaterials,earnedMissions,earnedWeaponTickets,clearRewardTickets,best,stones,recruited,act}));return;}
...
function updateGrowthLabels(){
  ...
  if($('#encounter-preparation'))$('#encounter-preparation').innerHTML=encounterPreparation(actFor(selectedAct),selectedParty,progression,HEROES);
...
function chooseTreeHero(id){if(game||!isHeroUnlocked(progression,id))return;treeHero=id;blessingSlot=0;const tier=talentNode(treeSelection,treeHero)?.tier;treeSelection=tier===3?'blessing1':tier===2?'ascension':'origin';renderTalent();$('#tree-feedback').textContent='';$(`[data-tree-hero="${id}"]`).focus({preventScroll:true});}
...
function recruitmentBanner(id){if(id==='mochinyafe')return `<section class="recruitment-banner" aria-label="仲間加入"><span class="portrait mochinyafe" role="img" aria-label="もちにゃふぇ"></span><div><small>NO LONGER ALONE</small><strong>もちにゃふぇが仲間になった！</strong><p>援護の「ふぇ〜」で雑魚を停止、ボスの攻撃・防御を低下。初期は最弱、Lv.50で最強へ育つ大器晩成型。パーティ編成から4人のうち最大2人を選べます。</p></div></section>`;const hero=HEROES.find(h=>h.id===id);return `<section class="recruitment-banner" aria-label="仲間加入"><span class="portrait ${id}" role="img" aria-label="${hero.name}"></span><div><small>${id==='omsolo'?'RESCUE COMPLETE':'REUNITED'}</small><strong>${id==='omsolo'?'オムソロが仲間になった！':'つきねこと、また一緒に！'}</strong><p>${id==='omsolo'?'緑の光刃で戦う近接の剣士。編成・祝福から仲間を入れ替えて、最大2人で出撃できます。':'大の仲良しの二人が再会。編成・育成・装備が解放されました。'}</p></div></section>`;}
function goMenu(result){
  const winningHero=game?HEROES[game.player.hero].id:HEROES[selectedHero].id;
  if(result)selectedAct=actFor(result.act).extra?result.act:Math.min(ACTS.length-1,result.act+1);goHome();equipmentHero=HEROES[selectedHero].id;treeHero=HEROES[selectedHero].id;treeSelection='origin';$('#home').classList.add('hidden');const chapter=chapterForAct(selectedAct),cleared=progression.story.actClears.slice(chapter.start,chapter.end+1).every(Boolean),act=actFor(selectedAct),actCleared=isActCleared(progression,selectedAct);
  $('#chapter-menu').innerHTML=`<div class="menu-backdrop"></div><div class="chapter-shell">
    <header class="chapter-header"><button id="menu-title" class="secondary">← タイトルへ戻る</button><h1>メニュー</h1><button class="icon-button" data-open="settings" aria-label="設定">${icon('settings')}</button></header>
    <section class="menu-preparation" aria-label="パーティの準備">
      <div class="menu-preparation-heading"><div><h2>冒険の準備</h2><p>仲間を編成して、挑戦するステージを選ぼう。</p></div><button id="menu-party-open" class="party-edit" data-open="party">${icon('swap')}<span><strong>パーティ編成</strong><small>メンバー・先頭を変更</small></span>${icon('chevron')}</button></div>
      <div class="menu-party"><span data-party-summary></span>${HEROES.map((h,i)=>`<button data-hero="${i}" aria-pressed="${selectedHero===i}" class="menu-hero ${selectedHero===i?'selected':''}">${portrait(i)}<strong>${h.name}<small data-hero-level="${h.id}">Lv.${heroLevel(h.id)}</small></strong><span data-party-role="${i}"></span></button>`).join('')}</div>
    </section>
    <nav class="menu-tabs" aria-label="メニュー切り替え"><button data-menu-tab="adventure" aria-pressed="true">${icon('compass')} ステージ選択</button><button data-menu-tab="growth" aria-pressed="false">${icon('spark')} キャラ育成</button><button data-menu-tab="talent" aria-pressed="false">${icon('link')} 成長ツリー</button><button data-menu-tab="missions" aria-pressed="false">${icon('trophy')} ミッション</button><button data-menu-tab="equipment" aria-pressed="false">${icon('gun')} 装備</button><button data-menu-tab="weapons" aria-pressed="false">${icon('star')} 武器ガチャ</button></nav>
    <div id="adventure-panel"><nav class="chapter-selector" aria-label="章を選ぶ">${CHAPTERS.map(c=>`<button data-chapter="${c.id}" aria-pressed="${chapter.id===c.id}" ${isActUnlocked(progression,c.start)?'':'disabled'}><small>CHAPTER 0${c.id+1}</small><strong>第${c.id+1}章</strong><span>${isActUnlocked(progression,c.start)?c.title:`第${c.id}章クリアで解放`}</span></button>`).join('')}</nav><div class="chapter-intro"><span class="eyebrow">CHAPTER 0${chapter.id+1} ${cleared?'· COMPLETE':''}</span><h2>${chapter.title}</h2><p>${chapter.summary}</p>${cleared?`<span class="chapter-complete">✓ 第${chapter.id+1}章クリア</span>`:''}</div>
    ${result?.recruited?recruitmentBanner(result.recruited):''}${result?`<section class="chapter-reward" aria-label="クリア報酬"><strong>${actLabel(result.act)}「${actFor(result.act).title}」クリア</strong><span>${result.best?'NEW BEST':'SCORE'} <b>${result.score.toLocaleString()}</b></span>${result.stones?`<span>覚醒の輝石 <b>+${result.stones}</b></span>`:''}<small>${earnedXpText(result.earnedXp)}</small><small class="result-materials">${materialsText(result.earnedMaterials)}</small>${ticketRewardText(result.earnedWeaponTickets,result.clearRewardTickets)}<small>${storageAvailable?'レベル・経験値・クリア報酬を保存しました':'この環境では記録を保存できません'}</small></section>${missionRunSummary(result.earnedMissions)}`:''}
    <div class="act-selector" role="group" aria-label="幕を選ぶ">${ACTS.filter(a=>a.chapter===chapter.id).map(a=>`<button data-act="${a.id}" aria-pressed="${selectedAct===a.id}" ${isActUnlocked(progression,a.id)?'':'disabled'}><small>ACT 0${a.number} · ${progression.story.actClears[a.id]?'CLEAR':isActUnlocked(progression,a.id)?'OPEN':'LOCKED'}</small><strong>第${a.number}幕</strong><span>${a.title}</span></button>`).join('')}</div>${extraStageSelector(chapter.id,selectedAct,progression)}<div class="act-intro"><h3>${act.extra?'エクストラ':`第${act.number}幕`} · ${act.title}</h3><p>${act.summary}</p><small>${act.extra?`全6WAVE・ミッションなし。次のクリアで武器ガチャ券${clearTicketReward(progression,selectedAct)}枚（初回10枚／以降2枚）。`:'全6WAVEクリアで毎回ガチャ券1枚確定。達成したミッション報酬もまとめて受け取れます。'}</small></div><div id="encounter-preparation"></div>${act.extra?extraDifficultyView(act):stageDifficultyView(difficulty,STAGE_MISSIONS.find(m=>m.act===selectedAct&&m.trial).id)}<div class="chapter-route">${act.stages.map((s,i)=>`<article class="chapter-stage" style="--stage-image:url('${s.image}')"><span class="stage-number">0${i+1}</span><div><small>${s.waves}${actCleared?' · CLEAR':''}</small><h3>${s.name}</h3><p>${s.note}</p><small class="terrain-badge">${fieldSummary(selectedAct,i)}</small></div><div class="terrain-preview">${mapSvg(fieldFor(selectedAct,i).rooms[0])}</div></article>`).join('')}</div><div class="chapter-bottom"><p class="departure-note">準備ができたら、選んだステージへ出発。</p><div class="chapter-start-panel"><button id="chapter-start" class="primary" data-mode="${act.extra?'extra':difficulty}"><span><strong>${act.extra?'エクストラ':`第${act.number}幕`}${actCleared?'をもう一度遊ぶ':'へ出発する'}</strong><small>${act.extra?'エクストラ · 最高難度':difficultyName(difficulty)}で出発</small></span>${icon('arrow')}</button>${act.extra?'':`<button id="chapter-story" class="secondary" ${actCleared?'':'disabled'}>この幕の物語を読み返す${actCleared?'':'（クリア後）'}</button>`}</div></div><p class="chapter-menu-note">${chapter.id===2?'第4幕の闇の王を倒し、最後の月の門を通ると、もちにゃふぇが加入。援護で育て、Lv.50の大きな成長を目指そう。第3章クリア後は、全EX最難関「夢蝕の王座」が解放。':chapter.id===1?'第4幕の巨神を倒してオムソロを救出。最後の月の門を通ると加入し、3人から最大2人を編成できます。クリア後は第3章・もちにゃふぇの国へ。':cleared?'第2章が解放されました。こむすびの願いを聞き、新しい救出の旅へ。':'最初はにゃんるな一人の旅。第4幕の最後のボス戦で親友のつきねこと再会し、クリア後に編成が解放。'}<br>全4幕・各幕6WAVE。敵を倒し、光る月の門へ進もう。</p></div>
    <section id="growth-panel" class="growth-panel hidden" aria-label="キャラ育成"><div class="growth-intro"><div><span class="eyebrow">CHARACTER GROWTH</span><h2>冒険は、仲間の力に。</h2><p>撃破したキャラに経験値100%、もう一人の出撃キャラに50%。援護で倒した場合も同じで、端数の0.5も保存します。戦闘不能の仲間も対象（未出撃・Lv.50は対象外）。</p></div><div class="stone-wallet">${materialArt('limitStone')}<span>覚醒の輝石<b id="limit-stones">${progression.inventory.limitStone}</b></span></div></div><p class="stone-source">各章の第4幕をクリアするたび、覚醒の輝石を1個獲得。第1章の第2〜4幕と第2・3章の高難度ミッション、第3章の金色のスライム討伐でも入手。<br>上限Lv.20 → 30 → 40 → 50。覚醒の輝石は1 → 2 → 4個、Lv.30へは★1素材、Lv.40・50へは★2素材も必要です。成長ツリーの「レベルの道」で必要数を確認できます。蓄積経験値も解放時に反映します。</p><p id="growth-feedback" class="growth-feedback" role="status"></p><div id="growth-cards" class="growth-cards">${growthCards(progression)}</div><div class="growth-note"><strong>クリスタルの祝福は、同じ幕の間ずっと有効。</strong><p>クリスタルを集めて選ぶ3択の祝福は、新しい出撃でリセット。キャラのレベル・経験値は残ります。${storageAvailable?'このブラウザに自動保存されます。':'この環境では記録を保存できません。'}</p></div></section>
    <section id="talent-panel" class="talent-panel hidden" aria-label="成長ツリー"><div class="talent-intro"><span class="eyebrow">CONSTELLATION GROWTH</span><h2>星をつなぎ、力を育てる。</h2><p>1段目で基礎を育て、2段目で必殺技を覚醒。スキルの星では戦闘の祝福候補を解放・付け替えできます。</p></div><p id="tree-feedback" class="growth-feedback" role="status"></p><div id="tree-content">${talentView(progression,treeHero,treeSelection,blessingSlot)}</div></section>
...
document.addEventListener('click',event=>{
  ...
  if(target.hasAttribute('data-prepare-challenge')&&!game){if(actFor(selectedAct).extra){selectedAct=chapterForAct(selectedAct).start;goMenu();}chooseDifficulty('hard');switchMenuTab('adventure');$('#stage-difficulty').scrollIntoView({block:'start'});$('#stage-difficulty [data-difficulty=hard]').focus({preventScroll:true});}
  ...
  if(target.dataset.treeTier&&!game){treeSelection=target.dataset.treeTier==='3'?'blessing1':target.dataset.treeTier==='2'?'ascension':'origin';renderTalent();$(`[data-tree-tier="${target.dataset.treeTier}"]`).focus({preventScroll:true});}
  ...
  if(['retry','chapter-start'].includes(target.id))start();
  ...
  if(['start','chapter-menu-open'].includes(target.id)){audio.init();audio.play('click');goMenu();}if(target.id==='menu-title')goHome();
...
function updateHud(){
  ...
  $('#objective span').textContent=game.act===11&&game.wave===6?(game.exitOpen?'もちにゃふぇを救出！ 最後の月の門へ':'闇の王を倒して、もちにゃふぇを救出'):game.rescue?.active?`オムソロを救出 · 残り ${Math.ceil(game.rescue.remaining)}秒`:game.rescue?.saved?'救出成功！ 最後の月の門へ':game.travelOpen==='stairs'?'青い階段から上のフロアへ':game.travelOpen==='branch'?'緑の通常ルート ／ 赤の強ボスルート':game.exitOpen?'光る月の門へ向かう':game.wave===6?'守護者を包む影をはらう':game.waveBreak>0?'次のWAVEへ · HP回復':'現れた魔物をすべて倒す';
