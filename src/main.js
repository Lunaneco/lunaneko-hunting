...
import {blessingSource,partyTitle,equipSkill} from './blessings.js';
import './style.css';
import './chapter.css';
...
import {EXTRA_COMBAT_HINT} from './extra-stages.js';
...
$('#app').innerHTML=`
  <section id="home" class="home" aria-label="タイトル画面">
    ...
    <footer class="home-footer"><span>NYANLUNA <i>×</i> TSUKINEKO</span><button id="chapter-menu-open">メニュー・育成</button><button data-open="guide">操作ガイド ${icon('arrow')}</button><span class="version">LUNANEKO ADVENTURE / 1.50.0</span></footer>
  </section>
...
function showResult(victory){
  ...
  const act=game.act,stones=victory&&actFor(act).number===4?1:0;record.runs++;if(victory){record.wins++;record.chapterOneCleared=progression.story.chapterOneCleared;if(stones)grantLimitStone(progression);}
