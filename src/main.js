...
$('#app').innerHTML=`
  <section id="home" class="home" aria-label="タイトル画面">
    ...
    <footer class="home-footer"><span>NYANLUNA <i>×</i> TSUKINEKO</span><button id="chapter-menu-open">メニュー・育成</button><button data-open="guide">操作ガイド ${icon('arrow')}</button><span class="version">LUNANEKO ADVENTURE / 1.31</span></footer>
  </section>
  ...
  <section id="ultimate-cutin" class="ultimate-cutin hidden" role="dialog" aria-modal="true" tabindex="-1" aria-label="必殺技の演出" aria-hidden="true">
    ...
    <div class="cutin-panel"><div class="cutin-portrait"><i class="cutin-orbit"></i><img class="cutin-face" alt="" draggable="false"></div>
      <div class="cutin-copy"><span class="cutin-role"></span><strong class="cutin-hero"></strong><small>CHARACTER ULTIMATE</small><h2 class="cutin-ability"></h2><p class="cutin-line"></p><div class="cutin-signal" aria-hidden="true">${'<i></i>'.repeat(9)}</div></div>
    </div>
  </section>
...
function closeDialog(){if(['upgrade','result','story'].includes(activeDialog))return;if(activeDialog==='pause'){if(!ultimatePresentation.resume()){game?.resume();voice.resume();}}$('#modal').close();activeDialog=null;tutorialView.render(game);resetInput();returnFocus?.focus?.();}
...
function ticketRewardText(count=0){return count?`<span class="run-ticket-reward">専用武器ガチャ券 +${count}枚</span>`:'';}
...
window.addEventListener('blur',()=>{voice.suspend();resetInput();if(game?.phase==='playing'||ultimatePresentation.active)pause();});
window.addEventListener('focus',()=>{if(activeDialog!=='pause')voice.resume();});
document.addEventListener('visibilitychange',()=>{if(document.hidden){voice.suspend();persistProgression();resetInput();if(game?.phase==='playing'||ultimatePresentation.active)pause();}else if(activeDialog!=='pause')voice.resume();});
window.addEventListener('pagehide',()=>{ultimatePresentation.cancel();voice.stop();persistProgression();});
const canvas=$('#scene');
...
if(import.meta.env.DEV){window.__LUNARIA_TEST__={get state(){return game?.snapshot()??{phase:'home'};},get stats(){return world?.stats();},get game(){return game;},get world(){return world;},get voice(){return voice;},get ultimatePresentation(){return ultimatePresentation;},get story(){return story;},start(){start({withStory:false});},step(seconds,input={x:0,z:0}){if(!game)return;for(let i=0;i<seconds*60;i++){game.tick(1/60,input);if(game.phase!=='playing')break;}handleEvents(game.drainEvents());updateHud();},skill(id){if(game?.chooseSkill(id)){if($('#modal').open)$('#modal').close();activeDialog=null;$('#modal').classList.remove('wide');handleEvents(game.drainEvents());}},home:goHome};}
