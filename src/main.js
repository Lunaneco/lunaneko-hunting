...
import {fieldSummary,updateTerrainUi} from './terrain-ui.js';
import './terrain.css';
...
import {createTitleVideo} from './title-video.js';
import './title-video.css';
...
`;
const titleVideo=createTitleVideo($('#title-video'),{motionEnabled:()=>settings.motion});
...
function chooseDifficulty(mode){
  ...
  difficulty=mode;if(activeDialog==='stage'){const focused=document.activeElement?.dataset.difficulty;$('#modal-content').innerHTML=dialogs.stage();if(focused)$(`[data-difficulty="${focused}"]`)?.focus({preventScroll:true});}updateDifficultyLabels();
...
function openDialog(type){
  if(activeDialog||(type==='party'&&game))return;audio.init();audio.play('click');returnFocus=document.activeElement;activeDialog=type;$('#modal-content').innerHTML=dialogs[type]()+(type==='stage'?'':`<button class="dialog-close icon-button" data-close aria-label="閉じる">${icon('close')}</button>`);if(type==='stage')$('#modal').setAttribute('aria-labelledby','stage-brief-title');else $('#modal').removeAttribute('aria-labelledby');$('#modal').showModal();if(type==='stage'){updateDifficultyLabels();$('#stage-brief-title').focus({preventScroll:true});}bindSettings();
}
...
function battleDetails(){
  const hero=HEROES[game.player.hero],progress=game.progressFor(game.player.hero),partner=game.partnerHero===null?null:HEROES[game.partnerHero],health=partner?game.healthFor(game.partnerHero):null;
  const boss=game.enemies.find(e=>e.type==='boss'),skills=Object.entries(game.skills).map(([id,rank])=>`${SKILLS.find(s=>s.id===id).name} Lv.${rank}`);
  return `<section class="battle-details" aria-label="戦闘の詳細"><h3>${game.layout.name}</h3><dl><div><dt>操作キャラ</dt><dd>${hero.name} · Lv.${progress.level}<br>HP ${Math.ceil(game.player.hp)} / ${game.player.maxHp}</dd></div><div><dt>経験値</dt><dd>${progress.level===LEVEL_RULES.maxLevel?'MAX LEVEL':`${progress.xp} / ${xpRequired(progress.level)}${progress.level>=levelCap(progress)?'（上限到達・蓄積中）':''}`}</dd></div><div><dt>援護</dt><dd>${partner?`${partner.name} · ${game.hasLivingPartner?`HP ${Math.ceil(health.hp)} / ${health.maxHp}<br>必殺ゲージ ${Math.floor(game.chargeFor(game.partnerHero))}%`:'戦闘不能'}`:'単独出撃'}</dd></div><div><dt>撃破数</dt><dd>${game.kills}体 · 最大CHAIN ${game.maxCombo}</dd></div><div><dt>祝福</dt><dd>${game.blessingTier}回獲得<br>クリスタル ${game.stageCrystals} / ${game.crystalGoal}</dd></div></dl>${skills.length?`<p>${skills.join(' ／ ')}</p>`:''}${boss?`<p>${boss.name}：${$('#boss-hud small').textContent}</p>`:''}<p class="pause-map-key">地図：白＝自分 ／ 赤＝敵 ／ 金のひし形＝ボス ／ 金の星＝レア ／ 大きな丸＝出口</p></section>`;
}
function pause(){if(!game||!(ultimatePresentation.active?ultimatePresentation.pause():game.pause()))return;voice.suspend();resetInput();activeDialog='pause';audio.setPaused(true);tutorialView.render(game,true);audio.play('click');$('#modal-content').innerHTML=`<div class="pause-symbol">${icon('moon')}</div><div class="modal-heading"><span class="eyebrow">TAKE A BREATH</span><h2>月明かりの下で、ひと休み。</h2><p>WAVE ${game.wave} / 6 <i>·</i> ${minutes(game.time)}</p></div><div class="run-materials"><small>この冒険で獲得した成長素材</small><p>${materialsText(game.earnedMaterials)}</p>${ticketRewardText(game.earnedWeaponTickets)}${game.earnedRareStones?`<p>レア討伐 ${game.goldenSlimeKills}体：覚醒の輝石 +${game.earnedRareStones}個</p>`:''}</div><button id="resume" class="primary">冒険をつづける ${icon('play')}</button><button id="quit" class="secondary">冒険を終了してメニューへ</button><p class="subtle">キャラの成長・獲得素材は保存済み。クリスタルと祝福は終了時にリセットされます。</p>${battleDetails()}<div class="enemy-guide"><h3>この幕のボス · ${game.actConfig.boss}</h3><p>${game.actConfig.extra?extraCombatHint(game.actConfig):BOSSES[game.actConfig.bossId].hint}</p><h3>新しい敵の見分け方</h3>${(game.actConfig.extra||game.actConfig.chapter>=1?enemyRosterForAct(game.act):['archer','mage','charger']).map(id=>`<p><b>${ENEMY_TYPES[id].name}</b> ${ENEMY_TYPES[id].hint}</p>`).join('')}</div>${game.actConfig.chapter===2&&!game.actConfig.extra?`<p class="subtle">${GOLDEN_SLIME.name}：${GOLDEN_SLIME.hint}</p>`:''}${currentMissions(progression,game)}`;$('#modal').showModal();}
function upgrade(){
...
function renderRewards(){
  ...
  if($('#missions-panel'))$('#missions-panel').innerHTML=missionsView(progression,actFor(selectedAct).extra?chapterForAct(selectedAct).start:selectedAct);
...
function goMenu(result){
  ...
  $('#chapter-menu').innerHTML=`<div class="chapter-shell">
    ...
    ${stageSelectionView(chapter.id,progression)}</div>
    <section id="growth-panel" class="growth-panel hidden" aria-label="キャラ育成"><div class="growth-intro"><div><span class="eyebrow">CHARACTER GROWTH</span><h2>冒険は、仲間の力に。</h2><p>冒険でレベルアップ。素材で、さらに強く。</p></div><div class="stone-wallet">${materialArt('limitStone')}<span>覚醒の輝石<b id="limit-stones">${progression.inventory.limitStone}</b></span></div></div><p id="growth-feedback" class="growth-feedback" role="status"></p><div id="growth-cards" class="growth-cards">${growthCards(progression)}</div>${storageAvailable?'':'<p role="status">この環境では記録を保存できません。</p>'}</section>
...
document.addEventListener('click',event=>{
  ...
  if(target.dataset.showMission&&!game){if(activeDialog==='stage')closeDialog();switchMenuTab('missions');const row=$(`[data-mission="${target.dataset.showMission}"]`);if(row?.closest('details'))row.closest('details').open=true;row?.scrollIntoView({block:'center',behavior:settings.motion?'smooth':'instant'});}
  if(target.hasAttribute('data-prepare-challenge')&&!game){if(actFor(selectedAct).extra){selectedAct=chapterForAct(selectedAct).start;goMenu();}chooseDifficulty('hard');switchMenuTab('adventure');openDialog('stage');}
  ...
  if(target.dataset.act!==undefined&&!game&&isActUnlocked(progression,Number(target.dataset.act))){selectedAct=Number(target.dataset.act);openDialog('stage');}
  if(target.id==='start'){audio.init();audio.play('click');goMenu();}if(target.id==='menu-title')goHome();
...
function handleEvents(events){
  ...
  for(const e of events){
    ...
    if(e.type==='bossAttack'&&!game.exitOpen&&game.phase==='playing')announce(`${e.label}。光る範囲を避けよう。`);
    if(e.type==='bossPhase'){toast(e.label);announce(e.label);}
    if(e.type==='rareSpawn'){audio.play('upgrade');toast('金色のスライム出現！');announce(GOLDEN_SLIME.hint);}
    ...
    if(e.type==='enemyIntro'){const enemy=ENEMY_TYPES[e.enemyType];toast(`${enemy.name}が出現`);announce(`${enemy.name}が出現。${enemy.hint}`);}
  }
...
async function boot(){
  try{world=new World(canvas,settings);document.body.classList.toggle('reduce-motion',!settings.motion);resize();requestAnimationFrame(frame);await Promise.all([world.ready,...[publicUrl('assets/title/adventure-poster.jpg'),publicUrl('assets/nyanluna-reference.png'),publicUrl('assets/tsukineko-reference.png'),publicUrl('assets/meadow.png')].map(src=>new Promise(resolve=>{const img=new Image();img.onload=resolve;img.onerror=resolve;img.src=src;}))]);titleVideo.setVisible(!$('#home').classList.contains('hidden'));$('#loading').classList.add('finished');setTimeout(()=>$('#loading').remove(),650);}
