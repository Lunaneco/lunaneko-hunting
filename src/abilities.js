...
export function ultimateFor(heroId,character){
  ...
  if(spec.kind==='barrage'){
    ...
    spec.note=spec.kind==='mochiLullaby'?`${spec.pulses}回のふぇ〜で周囲を包み、HPを${spec.heal}回復。雑魚を3秒停止、ボスの攻撃・防御を7秒間40%低下。`:spec.kind==='sanctuary'?`月の魔法陣で${spec.pulses}回の範囲攻撃。敵を減速し、発動時にHPを${spec.heal}回復。`:`周囲を${spec.pulses}回斬り払い、HPを${spec.heal}回復。発動中は無敵。移動・交代しても剣舞を続ける。`;
  }
  return spec;
}
