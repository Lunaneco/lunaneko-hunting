...
function offlineBundle(){let config;return {name:'lunaria-offline-bundle',apply:'build',configResolved(value){config=value;},async closeBundle(){
  ...
  await visit(dir);files.sort();const hash=createHash('sha256');hash.update('scoped-offline-v4'+base);for(const p of files){hash.update(relative(dir,p));hash.update(await readFile(p));}
